...
description: "Implement a Jira story in a local repository from an issue key or Jira URL. Use when the user asks Claude Code to implement, build, code, or complete a Jira story: read the full issue including acceptance criteria and comments, resolve the target repository, prepare the latest master safely with a feature branch or worktree, make the code changes, and run focused verification. Do not use for implementation delegation through APEX or for reviewing an existing pull request."
---
...
- leave changes local by default; do not commit, push, open a pull request, transition Jira, or edit the issue unless the user asks
