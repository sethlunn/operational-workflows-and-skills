...
description: "Implement a Jira story in a local repository from an issue key or Jira URL. Use when the user asks Claude Code to implement, build, code, or complete a Jira story: read the full issue including acceptance criteria and comments, resolve the target repository, prepare the latest master safely with a feature branch or worktree, make and verify the code changes, then commit, push, and open a draft pull request. Do not use for implementation delegation through APEX or for reviewing an existing pull request."
---
