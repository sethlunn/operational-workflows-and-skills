...
For .NET service work, assume APEX cannot build, run, or test the service. The handoff must require the APEX agent to state which build/test commands were not run and why, and must leave local review, runtime verification, documentation, and final PR readiness to this Claude session or a human.
...
Default to the `operations_workflow_claude` APEX template unless the user explicitly asks for the Codex/GPT template.
